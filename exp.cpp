...
void show_exp(HDC mdc, int experience, int x, int y, RECT rt) {
	...
	hfont = CreateFont(70 / 1500.0 * rt.right, 0, 0, 0, FW_BOLD, 0, 0, 0, HANGEUL_CHARSET, 0, 0, 0, VARIABLE_PITCH | FF_ROMAN, TEXT("맑은 고딕"));
...
void show_exp_add(HDC mdc, int prev_up, int x, int y, RECT rt) {
	...
	hfont = CreateFont(50 / 1500.0 * rt.right, 0, 0, 0, FW_BOLD, 0, 0, 0, HANGEUL_CHARSET, 0, 0, 0, VARIABLE_PITCH | FF_ROMAN, TEXT("맑은 고딕"));
	oldfont = (HFONT)SelectObject(mdc, hfont);
	SetBkMode(mdc, TRANSPARENT);
	SetTextColor(mdc, RGB(0, 0, 0));
